...
<body>
    <div class="d-flex" id="wrapper">
        ...
        <div id="content">
            <div>
                <h1>Top 3 Cursos con mas estudiantes</h1>
                <div id="contenedor"></div>

                <script>
                    document.addEventListener("DOMContentLoaded", function() {
                        fetch('http://127.0.0.1:8000/api/studentsCourses/top3')
                            .then(response => response.json())
                            .then(data => {
                                let contenedor = document.getElementById('contenedor');
                                data.forEach(item => {
                                    let texto = document.createElement('p');
                                    texto.textContent = `ID Curso: ${item.idCurso} - Curso: ${item.Nombre} - Cantidad: ${item.total_estudiantes}`;
