...
<body>
    <div class="d-flex" id="wrapper">
        ...
        <div id="content">
            <div>
                <h1>Listado de cursos de Estudiantes</h1>
                <div id="contenedor"></div>

                <script>
                    document.addEventListener("DOMContentLoaded", function() {
                        fetch('http://127.0.0.1:8000/api/studentsCourses/listaCursos')
                            .then(response => response.json())
                            .then(data => {
                                let contenedor = document.getElementById('contenedor');
                                data.forEach(item => {
                                    let texto = document.createElement('p');
                                    texto.textContent = `${item.id} - ${item.nombre} - ${item.apellido} - Cursos: {${item.cursos}}`;
